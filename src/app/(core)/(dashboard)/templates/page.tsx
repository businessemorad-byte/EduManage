    ...
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        ...
        <div className="flex gap-2">
          <button onClick={() => setShowVars(!showVars)} className="rounded-lg border border-zinc-200 px-4 py-2 text-sm font-medium text-zinc-700 hover:bg-zinc-50">
        ...
        <div className="rounded-lg border border-zinc-200 bg-white p-4">
          ...
          <div className="grid grid-cols-2 gap-2 sm:grid-cols-3">
            {AVAILABLE_VARIABLES.map((v) => (
              <div key={v.name} className="rounded-md bg-zinc-50 px-3 py-1.5 text-xs">
                <code className="font-mono text-violet-600">{"{{" + v.name + "}}"}</code>
        ...
        <div className="rounded-lg border border-zinc-200 bg-white p-5">
          ...
          <form onSubmit={handleSave} className="space-y-3">
            <div className="grid grid-cols-2 gap-3">
              <div><label className="mb-1 block text-xs font-medium text-zinc-600">Name *</label><input className={inputCls} value={form.name} onChange={(e) => setForm({ ...form, name: e.target.value })} required /></div>
              <div><label className="mb-1 block text-xs font-medium text-zinc-600">Code</label><input className={inputCls} value={form.code} onChange={(e) => setForm({ ...form, code: e.target.value })} placeholder="PAYMENT_REMINDER" /></div>
            </div>
            <div className="grid grid-cols-3 gap-3">
              <div><label className="mb-1 block text-xs font-medium text-zinc-600">Channel</label>
              ...
              <div><label className="mb-1 block text-xs font-medium text-zinc-600">Language</label>
              ...
              <div><label className="mb-1 block text-xs font-medium text-zinc-600">Status</label>
                <select className={inputCls} value={form.status} onChange={(e) => setForm({ ...form, status: e.target.value })}>
                  <option value="ACTIVE">Active</option><option value="INACTIVE">Inactive</option>
                </select>
              </div>
            </div>
            <div><label className="mb-1 block text-xs font-medium text-zinc-600">Subject</label><input className={inputCls} value={form.subject} onChange={(e) => setForm({ ...form, subject: e.target.value })} placeholder="Optional subject line" /></div>
            <div><label className="mb-1 block text-xs font-medium text-zinc-600">Body *</label><textarea rows={5} className={inputCls} value={form.body} onChange={(e) => setForm({ ...form, body: e.target.value })} required /></div>
            <div className="flex gap-2">
              <button type="submit" disabled={saving} className="rounded-lg bg-zinc-900 px-4 py-2 text-sm font-medium text-white hover:bg-zinc-800 disabled:opacity-50">{saving ? "Saving..." : "Save Template"}</button>
              <button type="button" onClick={() => { setEditing(null); setForm({ name: "", code: "", channel: "IN_APP", language: "en", subject: "", body: "", status: "ACTIVE" }); }} className="rounded-lg border border-zinc-200 px-4 py-2 text-sm font-medium text-zinc-700 hover:bg-zinc-50">Cancel</button>
            </div>
          </form>
        </div>
      )}
      ...
      <div className="w-72">
        <input className="w-full rounded-lg border border-zinc-200 bg-zinc-50 py-2 pl-3 pr-3 text-sm text-zinc-900 placeholder:text-zinc-400 transition-colors focus:border-brand-300 focus:bg-white focus:outline-none focus:ring-2 focus:ring-brand-100" value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Search templates..." />
      </div>
            ...
            <div key={t.id} className="flex items-center justify-between rounded-lg border border-zinc-200 bg-white px-4 py-3">
              <div className="min-w-0 flex-1">
                <div className="flex items-center gap-2">
                  ...
                  <span className="inline-flex items-center rounded-full bg-zinc-100 px-2 py-0.5 text-xs font-medium">{t.channel}</span>
                  <span className="inline-flex items-center rounded-full bg-zinc-100 px-2 py-0.5 text-xs font-medium">{t.language}</span>
                  {t.isSystem && <span className="inline-flex items-center rounded-full bg-purple-100 px-2 py-0.5 text-xs font-medium text-purple-700">System</span>}
                </div>
              ...
              <div className="ml-4 flex items-center gap-1">
                ...
                <button onClick={() => openEdit(t)} className="rounded-lg p-1.5 text-zinc-500 hover:bg-zinc-100" title="Edit"><Pencil className="h-3.5 w-3.5" /></button>
                <button onClick={() => duplicateTemplate(t)} className="rounded-lg p-1.5 text-zinc-500 hover:bg-zinc-100" title="Duplicate"><Copy className="h-3.5 w-3.5" /></button>
                {!t.isSystem && <button onClick={() => archiveTemplate(t.id)} className="rounded-lg p-1.5 text-zinc-500 hover:bg-zinc-100" title="Archive"><Archive className="h-3.5 w-3.5" /></button>}
              </div>
            </div>
          ))}
        </div>
