...
export default function SessionDetailPage() {
  ...
  if (error && !session) return <div className="rounded-xl border border-red-200 bg-red-50 p-6 text-center text-red-600">{error}</div>;
    ...
    <div className="animate-fade-in space-y-6">
      ...
      {conflicts && conflicts.length > 0 && (
        <div className="rounded-lg border border-amber-200 bg-amber-50 p-4">
          <p className="text-sm font-semibold text-amber-800">Schedule Conflicts</p>
          <ul className="mt-2 space-y-1">
            {conflicts.map((c, i) => <li key={i} className="text-sm text-amber-700">{c}</li>)}
          </ul>
        </div>
      )}
      ...
      <div className="grid gap-6 lg:grid-cols-3">
        <div className="space-y-4 lg:col-span-2">
          <div className="rounded-xl border bg-white p-6">
            <h3 className="mb-4 text-sm font-semibold text-zinc-900">Session Information</h3>
            {editing ? (
              <div className="space-y-4">
                <div>
                  <label className="mb-1 block text-sm font-medium text-zinc-700">Room</label>
                  <select value={editRoomId} onChange={(e) => setEditRoomId(e.target.value)} className="w-full rounded-lg border border-zinc-300 bg-white px-3 py-2 text-sm">
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    <label className="mb-1 block text-sm font-medium text-zinc-700">Teacher</label>
                    <select value={editTeacherId} onChange={(e) => setEditTeacherId(e.target.value)} className="w-full rounded-lg border border-zinc-300 bg-white px-3 py-2 text-sm">
                  ...
                  <div>
                    ...
                    <select value={editSubjectId} onChange={(e) => setEditSubjectId(e.target.value)} className="w-full rounded-lg border border-zinc-300 bg-white px-3 py-2 text-sm">
                ...
                <div>
                  ...
                  <select value={editGroupId} onChange={(e) => setEditGroupId(e.target.value)} className="w-full rounded-lg border border-zinc-300 bg-white px-3 py-2 text-sm">
                ...
                <div>
                  ...
                  <select value={editDay} onChange={(e) => setEditDay(e.target.value)} className="w-full rounded-lg border border-zinc-300 bg-white px-3 py-2 text-sm">
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    ...
                    <input type="time" value={editStart} onChange={(e) => setEditStart(e.target.value)} className="w-full rounded-lg border border-zinc-300 bg-white px-3 py-2 text-sm" />
                  </div>
                  <div>
                    <label className="mb-1 block text-sm font-medium text-zinc-700">End Time</label>
                    <input type="time" value={editEnd} onChange={(e) => setEditEnd(e.target.value)} className="w-full rounded-lg border border-zinc-300 bg-white px-3 py-2 text-sm" />
                  </div>
                </div>
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    <label className="mb-1 block text-sm font-medium text-zinc-700">Start Date</label>
                    <input type="date" value={editStartDate} onChange={(e) => setEditStartDate(e.target.value)} className="w-full rounded-lg border border-zinc-300 bg-white px-3 py-2 text-sm" />
                  </div>
                  <div>
                    <label className="mb-1 block text-sm font-medium text-zinc-700">End Date</label>
                    <input type="date" value={editEndDate} onChange={(e) => setEditEndDate(e.target.value)} className="w-full rounded-lg border border-zinc-300 bg-white px-3 py-2 text-sm" />
                  </div>
                </div>
                <div className="flex items-center gap-2">
                  ...
                  <label htmlFor="editRecurring" className="text-sm text-zinc-700">Recurring weekly</label>
                </div>
              </div>
            ) : (
          ...
          {session.schedule && !editing && (
            <div className="rounded-xl border bg-white p-6">
              <h3 className="mb-2 text-sm font-semibold text-zinc-900">Schedule</h3>
              <span className="text-sm text-zinc-600">{session.schedule.name}</span>
            </div>
          )}

          {session.module && !editing && (
            <div className="rounded-xl border bg-white p-6">
              <h3 className="mb-3 text-sm font-semibold text-zinc-900">Training Context</h3>
              <div className="space-y-2 text-sm">
                ...
                {session.module.program && (
                  <div className="flex items-center gap-3">
                    ...
                    <span className="text-zinc-600">{session.module.program.name}</span>
                  </div>
                )}
                {session.group && (
                  <div className="flex items-center gap-3">
                    <LinkIcon className="h-4 w-4 text-zinc-400" />
                    <a href={`/cohorts/${session.group.id}`} className="text-brand-600 hover:underline">
          ...
          <div className="space-y-2">
            {editing ? (
              <>
                ...
                <button onClick={handleCancelEdit} className="w-full rounded-lg border border-zinc-300 px-4 py-2.5 text-sm font-medium transition-colors hover:bg-zinc-50">
              ...
              <>
                <button onClick={() => setEditing(true)} className="w-full rounded-lg border border-zinc-200 px-4 py-2.5 text-sm font-medium text-zinc-700 transition-colors hover:bg-zinc-50">
