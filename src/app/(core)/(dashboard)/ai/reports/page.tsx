  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center gap-3">
        <select value={reportType} onChange={(e) => setReportType(e.target.value as "daily" | "weekly" | "monthly")} className="rounded-lg border border-zinc-300 bg-white px-3 py-2 text-sm">
          <option value="daily">Daily</option>
          <option value="weekly">Weekly</option>
          <option value="monthly">Monthly</option>
        </select>
        <button onClick={generate} disabled={loading} className="rounded-lg bg-zinc-900 px-4 py-2 text-sm font-medium text-white hover:bg-zinc-800 disabled:opacity-50">
      ...
      {report && (
        <div className="space-y-4">
          ...
          <div className="rounded-lg border border-zinc-200 bg-white p-4">
            ...
            <pre className="whitespace-pre-wrap text-sm text-zinc-700">{report.summary}</pre>
          </div>
        </div>
      )}
    </div>
