...
describe("Training Center - Attendance", () => {
  it("should mark attendance", async () => {
    vi.mocked(db.attendanceRecord.upsert).mockResolvedValue({ id: "att1", status: "PRESENT" } as never);
...
describe("Training Center - Assessments", () => {
  ...
  it("should list assessments", async () => {
    ...
    const result = await listAssessments("org1", { programId: "prog1" });
    expect(result).toHaveLength(2);
  });
...
writeFileSync("tests/unit/training-center.test.ts", content);
