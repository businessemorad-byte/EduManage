...
import type { PlatformPlan } from "@/lib/billing/platform-plans";
import { formatPlanAmount } from "@/lib/billing/platform-plans";
...
export default function PlatformBillingPage() {
  ...
  async function savePlan(plan: Plan) {
    ...
    setSaveError(null);
    try {
      const res = await fetch("/api/platform/billing/plans", {
        method: "PATCH",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          planId: plan.id,
          priceMonthlyMad: plan.priceMonthlyMad === null ? null : Number(plan.priceMonthlyMad),
            ...
            <tbody>
              {plans.map((plan) => (
                <tr key={plan.id} className="border-b last:border-0">
                    ...
                    <input
                      ...
                      onChange={(e) =>
                        setPlans((prev) =>
                          prev.map((p) =>
                            p.id === plan.id
                              ? { ...p, priceMonthlyMad: e.target.value === "" ? null : Number(e.target.value) }
                              : p
                          )
                        )
                      }
                  ...
                  <td className="p-3">{formatPlanAmount(plan.aiCreditsMonthly)}</td>
                  <td className="p-3">
                    <input
                      type="checkbox"
