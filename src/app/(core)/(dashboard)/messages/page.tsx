  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        ...
        <button onClick={() => { setComposing(true); setSelectedMessage(null); }} className="flex items-center gap-2 rounded-lg bg-zinc-900 px-4 py-2 text-sm font-medium text-white hover:bg-zinc-800 dark:bg-zinc-100 dark:text-zinc-900">
      ...
      <div className="flex gap-2 border-b border-zinc-200 dark:border-zinc-800">
        {(["inbox", "sent"] as const).map((t) => (
          <button key={t} onClick={() => { setTab(t); setSelectedMessage(null); setComposing(false); }} className={`border-b-2 px-4 py-2 text-sm font-medium capitalize ${tab === t ? "border-zinc-900 dark:border-zinc-100" : "border-transparent text-zinc-500"}`}>
            {t}
          </button>
        ))}
      </div>
      ...
      {composing && (
        <div className="rounded-lg border border-zinc-200 bg-white p-5 dark:border-zinc-800 dark:bg-zinc-950">
          ...
          {sendError && <div className="mb-3 rounded-lg bg-red-50 p-2 text-sm text-red-700 dark:bg-red-900/20 dark:text-red-400">{sendError}</div>}
          {sendSuccess && <div className="mb-3 rounded-lg bg-emerald-50 p-2 text-sm text-emerald-700 dark:bg-emerald-900/20 dark:text-emerald-400">{sendSuccess}</div>}
          <form onSubmit={handleSend} className="space-y-3">
            <div>
              <label className="mb-1 block text-sm font-medium text-zinc-700 dark:text-zinc-300">Recipient *</label>
            ...
            <div className="flex gap-2">
              <button type="submit" disabled={sending} className="rounded-lg bg-zinc-900 px-4 py-2 text-sm font-medium text-white hover:bg-zinc-800 disabled:opacity-50 dark:bg-zinc-100 dark:text-zinc-900">
                {sending ? "Sending..." : "Send Message"}
              </button>
              <button type="button" onClick={() => setComposing(false)} className="rounded-lg border border-zinc-200 px-4 py-2 text-sm font-medium text-zinc-700 hover:bg-zinc-50 dark:border-zinc-700 dark:text-zinc-300">
                Cancel
              </button>
            </div>
          </form>
      ...
      {!composing && selectedMessage && (
        <div className="rounded-lg border border-zinc-200 bg-white p-5 dark:border-zinc-800 dark:bg-zinc-950">
          ...
          <p className="whitespace-pre-wrap text-sm text-zinc-700 dark:text-zinc-300">{selectedMessage.content}</p>
          <div className="mt-4">
            <button onClick={() => { setComposing(true); setSelectedMessage(null); }} className="rounded-lg border border-zinc-200 px-4 py-2 text-sm font-medium text-zinc-700 hover:bg-zinc-50 dark:border-zinc-700 dark:text-zinc-300">
              Reply
            </button>
          </div>
        </div>
          ...
          {messages.map((m) => (
            <div
              ...
              className={`cursor-pointer rounded-lg border px-4 py-3 transition-colors ${
                m.read ? "border-zinc-200 bg-white hover:bg-zinc-50 dark:border-zinc-800 dark:bg-zinc-950 dark:hover:bg-zinc-900" : "border-blue-200 bg-blue-50/50 hover:bg-blue-50 dark:border-blue-900 dark:bg-blue-950/20"
              }`}
            >
              ...
              <p className="mt-1 text-sm text-zinc-600 dark:text-zinc-400 line-clamp-2">{m.content}</p>
            </div>
          ))}
        </div>
      )}
